...
const refreshData = async () => {
  const response = await axios.get(
    `/gallery?page=${currentPage.value}&limit=${itemsPerPage}`
  );
...
const selectedIndexChild = ref(0);
const indexParent = ref(0);
const holdIndex= ref(0);
const viewImagesGallery = (index, imagesLength) => {
  if (selectedIndexChild.value !== imagesLength - 1) {
    indexParent.value = index;
    selectedIndexChild.value++;
  }
};
  ...
  <div class="py-3 px-4 lg:px-[50px] xl:px-32 mt-[60px] lg:mt-24 xl:mt-24">
    ...
    <main class="flex w-full">
      <Left-Sidebar>
        ...
        <Post-Content :showAllpost="showAllpost">
          <tr
          ...
          >
            ...
            >
              ...
              >
                <div
                  class="relative flex flex-shrink-0 w-full max-h-auto"
                  v-for="(image,indexChild ) in parseImages(gallery.images)"
                  :key="image"
                  v-show="index===indexParent? indexChild===selectedIndexChild: indexChild===0"
                >
                  <img
                    class="rounded-lg w-full"
                    :src="`https://storage.googleapis.com/mcm-chuch.appspot.com/${image}`"
                    alt="Image 1"
                    loading="lazy"
                  />
                </div>
    ...
    <footer>
      <div
        class="flex py-5 mt-5 m-auto text-gray-800 text-sm flex-col items-center border-t max-w-screen-xl"
      >
