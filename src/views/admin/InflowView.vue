...
const refreshData = async () => {
  try {
    const response = await axios.get(
      `/api/entry?page=${currentPage.value}&limit=${itemsPerPage.value}&searchBy=${searchBy.value}&searchByDate=${searchByDate.value}`
    );
    if (response.data.success) {
      entries.value = response.data.entries; // Adjusted to 'entries'
    } else {
...
// Watch for currentPage and searchBy changes to refresh data
watch([currentPage, searchBy, searchByDate], refreshData);
...
<template>
  <main>
    ...
    <main class="pb-3 px-4">
      <h1 class="text-2xl font-semibold mb-4">Cash inflow</h1>
              ...
              <table class="min-w-full rounded-xl">
                ...
                <tbody class="divide-y divide-gray-300">
                  ...
                  >
                    ...
                    <td
                      class="p-2 sm:p-5 md:p-5 lg:p-5 xl:p-5"
                      v-if="access_control === 'Bookkeeper'"
                    >
