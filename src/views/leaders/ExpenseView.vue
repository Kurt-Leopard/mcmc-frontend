...
let allocatedBudget = { value: 0 };

refreshAllocation().then(() => {
  let totalAmount = 0;
  allocation.value.forEach(element => {
    totalAmount += element.amount;
  });
  allocatedBudget.value = totalAmount;
});
...
<template>
  <main class="py-3 px-4 lg:px-[50px] xl:px-32 mb-12 mt-[115px] lg:mt-32 xl:mt-32 flex justify-center">
    <div class="my-6 w-full">
      <!--  -->
      <div class="md:p-6 lg:p-6 xl:p-6 bg-background rounded-lg md:shadow-md lg:shadow-md xl:shadow-md">
        <div>
          <div class="w-full hidden md:grid lg:grid xl:grid grid-cols-3 gap-4">
            <div
              ...
              <div v-for="(allocated, index) in allocation" :key="index" class="text-right text-3xl font-semibold">
                {{ allocated.amount }}
              </div>
            </div>
            ...
            <div
              ...
              <div class="flex justify-between items-center mb-4">
                <span class="text-sm uppercase tracking-wide">Remaining Balance</span>
                <div class="text-xs">MCMC</div>
              </div>
              <div class="text-right text-3xl font-semibold">{{ allocatedBudget.value }}
              </div>
            </div>
            ...
            <div
              ...
              <div class="flex justify-between items-center mb-4">
                <span class="text-sm uppercase tracking-wide">Total Change</span>
                <div class="text-xs">MCMC</div>
              </div>
      ...
      <div class="w-full lg:flex xl:flex items-center justify-between">
        <section class="md:flex lg:flex xl:flex items-center gap-2 w-full">
          ...
          <input v-model="searchByDate" @input="refreshData" type="month" placeholder="Search by month and year"
            class="border border-gray-300 p-2 rounded lg:w-1/2 xl:w-1/2 w-full" />
        </section>
        <section class="lg:w-1/3 xl:w-1/3 flex items-center my-2 lg:justify-even xl:justify-even lg:px-6 xl:px-6 gap-6">
          <select v-model="typeOfExpense"
            class="py-3 px-4 pe-9 block w-full border border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900   dark:placeholder-neutral-500 dark:focus:ring-neutral-600">
            <option value="Bill">Bill</option>
            <option value="Ministry">Ministry</option>
            <option value="Honorarium">Honorarium</option>
            <option value="Others">Others</option>
          </select>
          <a class="text-gray-500 flex items-center justify-center px-3.5 hover:text-gray-700 relative">
            <button @click="viewRequest">
              <i class="fas fa-clipboard"></i></button><small v-if="expensesRequest.length !== 0"
      ...
      <div :class="errorResponse
        ...
        ">
        <div v-if="errorResponse" class="flex items-center justify-center w-full h-1/2">
          <div class="text-center">
            <i class="fas fa-database text-[100px] text-gray-200"></i>
            <h1 class="py-3">No data available yet.</h1>
          </div>
        </div>
        <div v-show="!errorResponse" class="w-full border border-gray-300 p-6" v-for="(expense, index) in expenses"
          :key="index">
          <div class="px-2 py-2">
            <h2 class="text-lg font-mono font-bold text-center border-b pb-2 mb-4">
          ...
          <div class="px-2">
            ...
            <div class="flex justify-between mb-4">
              ...
              <span
                :class="expense.spent !== null ? ' bg-green-100 rounded-md p-1 text-xs font-mono' : 'text-xs font-mono'">{{
                  expense.total_change || '0.00' }}</span>
            </div>
            <div class="border-t border-gray-300 my-3"></div>
            <div class="flex justify-between items-center w-full" v-if="access_control === 'Bookkeeper'">
              <span class="text-sm font-bold font-mono" v-if="expense.spent === null">Change: {{ expense.spent }}</span>
              <button v-if="expense.spent === null" @click="changeLog(expense)" type="button"
      ...
      <div class="flex justify-center items-center text-gray-500 space-x-4 my-4 w-full">
        <button @click="previousPage" :disabled="currentPage <= 1" class="hover:text-gray-800 cursor-pointer text-sm">
          Prev
        </button>
        <small>Page {{ currentPage }}</small>
        <button @click="nextPage" :disabled="expenses.length < itemsPerPage"
    ...
    <ChangeLogComponent v-if="isExpenseLog" @closeExpenseLog="closeExpenseLog" @refreshData="refreshData" />
  </main>
</template>
